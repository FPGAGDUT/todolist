...
class LoginWindow(QtWidgets.QWidget):
    ...
    def init_ui(self):
        ...
        self.remember_me = QtWidgets.QCheckBox("记住我")
    ...
    def handle_login_result(self, result):
        ...
        if result.get('success'):
            ...
            if self.remember_me.isChecked():
                # 保存用户名到配置文件中
                self.save_credentials(self.username_input.text().strip())
    ...
    def save_credentials(self, username):
        ...
        settings.setValue("rememberUser", True)
    
    def load_saved_credentials(self):
        """加载保存的凭据"""
        settings = QtCore.QSettings("AITodoList", "UserPrefs")
        remember = settings.value("rememberUser", False, type=bool)
