...
class NetworkManager:
    ...
    def update_task_direct(self, task_id, task_data):
        """直接更新任务(不使用批量操作)"""
        # 更新本地存储
        ...
        if self.is_online and not self.local_first:
            result = self._make_request("put", f"tasks/{task_id}", data=task_data)
            self.logger.info(f"直接更新任务 {task_id}: {result}")
            return result
    ...
    def create_task(self, task_data):
        ...
        task_data["id"] = temp_id
        task_data["created_at"] = datetime.now().isoformat()
        
        # 保存到本地存储
        self.local_storage.update_task(temp_id, task_data)
        ...
        return {"success": True, "data": {"id": temp_id}}
